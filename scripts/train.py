    ...
    *,
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            size = len(batch["rows"])
            for key, value in (("loss_total", losses.total), ("loss_seq", losses.sequence), ("loss_answerability", losses.answerability)):
...
def _gold_answer_span(batch: dict, index: int, eos_id: int) -> list[int] | None:
    answer_tokens = _gold_answer_tokens(batch, index, eos_id)
    context_positions = torch.where(batch["context_mask"][index] & batch["source_valid"][index])[0].tolist()
    source_ids = batch["source_ids"][index].tolist()
    for offset in range(len(context_positions) - len(answer_tokens) + 1):
        positions = context_positions[offset : offset + len(answer_tokens)]
        if [source_ids[position] for position in positions] == answer_tokens:
            return positions
    return None
...
def evaluate_generated(
    ...
    by_question_type: dict[str, dict[str, float]] = defaultdict(lambda: {"rows": 0, "f1": 0, "pointer": 0, "start": 0})
    by_context_length: dict[str, dict[str, float]] = defaultdict(lambda: {"rows": 0, "f1": 0, "start": 0, "span_mass": 0})
    by_lexical_overlap: dict[str, dict[str, float]] = defaultdict(lambda: {"rows": 0, "f1": 0, "start": 0})
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            for index, (predicted_ids, row) in enumerate(zip(predicted, batch["rows"])):
                ...
                gold_span = _gold_answer_span(batch, index, tokenizer_info.eos_id)
...
def main() -> None:
    ...
    parser.add_argument("--curriculum", choices=["standard", "entity_binding", "a1c", "a1d", "a2a"], default="standard")
    ...
    copy_only = True
    pointer_lambda = 1.0 if args.lambda_pointer_position is None and args.curriculum == "a2a" else args.lambda_pointer_position or 0.0
    ...
    if args.checkpoint:
        ...
        if args.curriculum in {"entity_binding", "a1c", "a1d", "a2a"} and not args.resume:
    ...
    logger = WandbLogger(
        ...
        config={
            **cfg.to_dict(),
            ...
            "dataset_version": "squad2_answerable_plus_procedural_replay" if args.curriculum == "a2a" else "synthetic_procedural_copy_v6",
            "training_data": "70% SQuAD2 answerable, 20% A1 replay, 10% A1d replay" if args.curriculum == "a2a" else "online deterministic procedural rows; no replayed corpus",
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        for micro_step in range(args.grad_accum):
            ...
            batch = online_batch(
                ...
                cfg=cfg,
                curriculum=args.curriculum,
                curriculum_step=step if args.curriculum in {"a1c", "a1d", "a2a"} else step - start_step,
        ...
        if step % args.eval_every == 0 or step == args.steps:
            ...
            for name, loader in val_loaders.items():
                metrics = evaluate(model, loader, device, 0.0, copy_only=copy_only, eos_id=cfg.eos_id)
            ...
            logger.table(
                ...
                [
                    ...
                    "greedy_correct", "eos_correct", "pointer_trajectory",
                ],
                qualitative_rows,
                step=step,
            )
