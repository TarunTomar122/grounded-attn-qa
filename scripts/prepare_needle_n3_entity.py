...
def main() -> None:
    ...
    manifest = {
        "purpose": "N3 entity-binding bridge: distinguish a present entity from same-relation competing entities before official negatives.",
        "composition": {"n2_answerable": 0.60, "entity_answerable": 0.10, "cross_pair": 0.15, "entity_missing": 0.15},
    ...
    for split, seed in (("train", 41), ("validation", 97)):
        ...
        entity_positive_rows = round(len(n2["source_ids"]) / 6)
        negative_rows = round(len(n2["source_ids"]) / 4)
        ...
        combined = {
            key: torch.cat((n2[key], entity_positive_tensors[key], cross[key], entity_negative_tensors[key]))
            for key in n2
        }
